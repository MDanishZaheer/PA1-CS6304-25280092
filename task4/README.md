...
This folder implements the required CIFAR-10/CIFAR-100 open-set recognition study with Vanilla, GCSC, and PROSER.
...
The CIFAR ResNet-18 uses a 3x3 stride-1 first convolution and no initial max-pooling. Vanilla and GCSC use SGD with learning rate 0.1 for 100 epochs. PROSER uses learning rate 0.001 for 50 epochs. All use momentum 0.9, weight decay 0.0005, cosine decay, batch size 128, and seed 6304. Checkpoints are selected only by CIFAR-10 validation accuracy.
...
The frozen Vanilla outputs are scored with MSP, MLS, Energy, and shared-diagonal Mahalanobis distance. GCSC and PROSER use MLS for the common trained-model comparison. PROSER also uses its calibrated strongest-dummy probability minus maximum-known probability score.
...
2. Fixed CIFAR-10 split preparation.
3. Vanilla, GCSC, and PROSER training.
